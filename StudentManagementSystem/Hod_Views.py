...
from app.models import Course,Session_Year,CustomUser,Student,Staff,Subject,Session_Year,Staff_Notification,Staff_leave,Staff_Feedback,Student_Notification,Student_Feedback,Student_leave,Attendance,Attendance_Report
...
def ADD_STAFF(request):
    if request.method == 'POST':
        ...
        if CustomUser.objects.filter(username = username).exists():
            ...
            staff.save()
            messages.success(request,'Staff Add Successfull !!')
            return redirect('add_staff')
...
def UPDATE_STAFF(request):
    if request.method == 'POST':
        ...
        staff.save()
        messages.success(request,'Staff Updated SuccessFull')
...
def STAFF_FEEDBACK(request):
     ...
     feedback_histroy = Staff_Feedback.objects.all().order_by("-id")[0:5]
     context = {
          'feedback' : feedback,
          'feedback_histroy':feedback_histroy
     }
     return render(request,'Hod/staff_feedback.html',context)

def STUDENT_FEEDBACK(request):
    feedback = Student_Feedback.objects.all()
    feedback_histroy = Student_Feedback.objects.all().order_by("-id")[0:5]
    context = {
          'feedback' : feedback,
          'feedback_histroy':feedback_histroy
    }
